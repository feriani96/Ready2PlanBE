...
export class EntrepriseComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['id', 'nom_etp', 'pays', 'description', 'devise'];
  ...
  trackId = (_index: number, item: IEntreprise): string => this.entrepriseService.getEntrepriseIdentifier(item);

  search(query: string): void {
    if (query && EntrepriseComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = '';
    }
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (EntrepriseComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.entrepriseService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
